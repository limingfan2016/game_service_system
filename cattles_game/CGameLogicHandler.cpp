...
int CGameLogicHandler::init(CCattlesMsgHandler* pMsgHandler)
{
...
void CGameLogicHandler::setInvitationStatus(const SCattlesRoomData& roomData, google::protobuf::RepeatedPtrField<com_protocol::InvitationInfo>* invitationInfoList)
{
	const unsigned int minNeedGold = m_msgHandler->getMinNeedPlayerGold(roomData);
	...
	{
		status = it->status();
		if (it->dynamic_info().game_gold() > minNeedGold              // 金币满足
...
void CGameLogicHandler::onPlayerDismissRoom(unsigned int timerId, int userId, void* param, unsigned int remainCount)
{
	...
	{
		// 结算记录
		m_msgHandler->getGameProcess().cardCompleteSettlement(strHallId, strRoomId, *roomData, false);

		com_protocol::ClientConfirmDismissRoomNotify cdrNtf;
		m_msgHandler->sendPkgMsgToRoomPlayers(*roomData, cdrNtf, COMM_PLAYER_CONFIRM_DISMISS_ROOM_NOTIFY,
											  "confirm dismiss room", "");
...
void CGameLogicHandler::refusePlayerInvitationNotify(const char* data, const unsigned int len, unsigned int srcSrvId, unsigned short srcModuleId, unsigned short srcProtocolId)
{
	...
	const SCattlesRoomData* roomData = m_msgHandler->getRoomDataById(cud->roomId, cud->hallId);
...
void CGameLogicHandler::playerAskDismissRoom(const char* data, const unsigned int len, unsigned int srcSrvId, unsigned short srcModuleId, unsigned short srcProtocolId)
{
	...
	const com_protocol::ClientRoomPlayerInfo* playerInfo = (roomData != NULL) ? roomData->getPlayerInfo(cud->userName) : NULL;
	if (playerInfo == NULL || playerInfo->status() < com_protocol::ERoomPlayerStatus::EPlayerSitDown
	...
	{
		rsp.set_result(CattlesAskDismissRoomFrequently);
	}
	...
	m_msgHandler->sendPkgMsgToRoomPlayers(*roomData, dismissRoomNtf, COMM_PLAYER_DISMISS_ROOM_ASK_NOTIFY,
                                          "player ask dismiss room", "", com_protocol::ERoomPlayerStatus::EPlayerSitDown);
...
void CGameLogicHandler::playerAnswerDismissRoom(const char* data, const unsigned int len, unsigned int srcSrvId, unsigned short srcModuleId, unsigned short srcProtocolId)
{
	...
	m_msgHandler->sendPkgMsgToRoomPlayers(*roomData, ntf, COMM_PLAYER_DISMISS_ROOM_ANSWER_NOTIFY,
                                          "player answer dismiss room", "", com_protocol::ERoomPlayerStatus::EPlayerSitDown);
	...
	{
		// 任意一个玩家不同意则不可以解散房间
        m_msgHandler->getGameProcess().refuseDismissRoomContinueGame(cud, *roomData);
	}
}
