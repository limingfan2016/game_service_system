...
void CHttpConnectMgr::run()
{
	...
	// 忽略管道SIGPIPE信号
	CProcess::installSignal(SIGPIPE, NPlatformService::SignalHandler);
...
void CHttpConnectMgr::onActiveConnect(uint32_t eventVal, ConnectData* conn)
{
	...
	if (err == EINPROGRESS)
	{
		...
		OptWarnLog("connect http host time out, flag = %d, key = %s", conn->keyData.flag, conn->keyData.key);
	...
	if (connData->sslConnect != NULL)
	{
		do
		{
			...
			if (time(NULL) > connData->lastTimeSecs)  // 超时则退出
			{
				...
				OptWarnLog("connect https to do SSL operation host time out, flag = %d, key = %s", connData->keyData.flag, connData->keyData.key);
			}
